...
class FamilyParser extends BaseParser {
    ...
    getChildren() {
        ...
        while ((m = this.unparsed.match(/^.+?\n[=-]{30,}\n/s))) {
            const block = m[0];
            this.chopUnparsedStart(block.length);
            const [, divider, personId, sex] = block.match(/^[ \d]\d ([|?]) NAME: .+\(#(\d+)\)\n ([FM ]) /);
            if (!divider) {
            ...
            if (uncertain) {
                console.warn(`Uncertain child ${personId} for ${this.getFamilyId()}`);
            }
        }
    ...
    getParentIds() {
        return ['HUSBAND', 'WIFE'].map(key => this.getProperty(key))
            .filter(value => !!value);
    }
